...
async function transcribeAudioGroq(audioPath) {
  try {
    ...
    if (error.response?.status === 400) {
      throw new Error(
        `Audio transcription failed: Bad request - ${
...
function saveChatHistory(history) {
  try {
    ...
    fsSync.writeFileSync(
      CHAT_HISTORY_FILE,
      JSON.stringify(trimmedHistory, null, 4),
      "utf8"
    );
  } catch (error) {
...
function analyzeConversationContext(chatHistory) {
  ...
  if (recentMessages.length > 0) {
    ...
    if (recentUserMessages.length > 0) {
      contextSummary = `Recent conversation topics: ${
        topics.join(", ") || "general conversation"
      }. `;
      contextSummary += `User has been discussing: ${recentUserMessages
        .map((msg) => msg.english)
        .join(", ")}.`;
    }
  }
...
function generateContextualSystemPrompt(context) {
  ...
  if (context.messageCount === 0) {
    basePrompt +=
      "This is the start of a new conversation. Greet the user warmly in Chinese. ";
  } else if (context.conversationStyle === "greeting") {
    basePrompt += "Continue the greeting conversation naturally. ";
  } else {
    basePrompt += `Continue the conversation naturally. Recent topics have included: ${
      context.recentTopics.join(", ") || "general conversation"
    }. `;
  }
  ...
  basePrompt +=
    "Make your response relevant to what the user said and the conversation context. ";
  basePrompt +=
    "After your Mandarin response, on a new line, provide the English translation of your Mandarin response, like this: English translation: [Your English translation here]";
...
async function getAIResponseWithAudio(userText, chatHistory = null) {
  try {
    ...
    return {
      ...
      audio_url: audioFilenameToReturn
        ? `/static/audio/${audioFilenameToReturn}`
        : null,
      context: context, // Include context in response for debugging
    };
  } catch (error) {
    console.error(
      `Overall error in getAIResponseWithAudio:`,
      error.response?.data || error.message
    );
...
app.post("/interact", upload.single("audio_file"), async (req, res) => {
  try {
    if (!req.file) {
      return res
        .status(400)
        .json({ error: "No audio file provided or invalid audio file type." });
    }
    ...
    if (!userTranscribedText) {
      ...
      console.log(
        "Transcription failed or empty, returning contextual fallback response."
      );
    ...
    let aiResponseData = await getAIResponseWithAudio(
      userTranscribedText,
      chatHistory
    );
    console.log(
      `AI (Groq response package): ${JSON.stringify(aiResponseData)}`
    );
    ...
    if (!aiResponseData) {
      ...
      aiResponseData = {
        ...
        english:
          context.messageCount === 0
            ? "Hello! Any questions?"
            : "Sorry, AI response processing failed.",
        audio_url: null,
      };
    }
    ...
    try {
      ...
      console.error(
        `Error removing temporary file ${tempAudioPath}: ${removeError}`
      );
    }
...
app.use((error, req, res, next) => {
  ...
  if (error.message === "Invalid audio file type") {
    return res
      .status(400)
      .json({
        error: "Invalid audio file type. Please upload a valid audio file.",
      });
  }
